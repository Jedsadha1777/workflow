// Luckysheet to HTML Converter - 100% Complete
function luckysheetToHtml() {
    ...
    sheets.forEach((sheet, sheetIndex) => {
        if (sheetIndex > 0) {
            fullHtml += '<div style="page-break-before: always; margin-top: 30px;"></div>';
        }
        
        fullHtml += '<h3>' + escapeHtml(sheet.name) + '</h3>';
        ...
        const borderInfo = config.borderInfo || [];
        const condFormat = config.condFormat || [];
...
function getCellBorders(r, c, borderInfo, cell) {
    ...
    borderInfo.forEach(border => {
        ...
        if (rangeType === 'cell') {
            ...
            if (row_index === r && col_index === c) {
                applyBorderToCell(borders, borderType, style, color);
            }
            ...
            if (r >= row[0] && r <= row[1] && c >= column[0] && c <= column[1]) {
                if (borderType === 'border-all') {
                    borders.top = getBorderString(style, color);
                    borders.bottom = getBorderString(style, color);
                    borders.left = getBorderString(style, color);
                    borders.right = getBorderString(style, color);
    ...
    if (cell.bd) {
        ...
        if (cell.bd.d) {
            diagonalBorder = {
                type: cell.bd.d.t, // 1 = top-left to bottom-right, 2 = bottom-left to top-right, 3 = both
                style: cell.bd.d.s,
                color: cell.bd.d.c
            };
        }
    ...
    if (!borderStyle) {
        borderStyle = 'border: 1px solid #d0d0d0; ';
    }
    
    return { borderStyle: borderStyle, diagonalBorder: diagonalBorder };
}

function applyBorderToCell(borders, borderType, style, color) {
